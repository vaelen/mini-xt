...
- [ ] **Step 4: Inward lines.** `X_IRQ2..8` → `EXT_IRQ2..8` and `X_DRQ1..3` → `EXT_DRQ1..3` through 2× `mini-xt:74HCT244` value `"74LVC244A"` at `+3V3` (5V-tolerant inputs), with 100k pull-DOWNs on every `X_` input so unplugged lines read 0. IOCHRDY and `~{IOCHCK}`: one open-drain buffer per line — place the existing `mini-xt:74LVC2G06` body with value `"74LVC2G07"` (Task 2 bound that (lib_id, value) pair in parts.py; there is deliberately no separate 2G07 symbol — add the required deviation comment) — driving the INTERNAL net from the `X_` line, pull-ups on the X_ side — wired-AND semantics, no contention:
