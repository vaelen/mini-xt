...
---
**2026-07-14 (third pass): disable jumper centralized.** JP1 and R6 deleted;
U3's '125 OEs now take DIS_NIC from `addr_decode` JP5 (mxbus.PRIV_DIS --
same active-low-enable polarity the local ~{NET_EN} had, but the sense at
the jumper is inverted: ENABLED by default, fit JP5 to disable). The '125
and R7 stay local: the disable must reach inside the 5V island (RTL8019AS
self-decodes; there is no external CS to gate).
