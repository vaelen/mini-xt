...
Design doc S3/S4/S7. This is the motherboard's hard-real-time critical path:
  ...
  * 3x 74HC573 address latches (AD0-7 + A8-A19 -> A0-A19), gated by ALE
  * 74HC245 data transceiver (D0-D7)
  * 74HCT138 + 74HC00 SRAM chip-select decode (Y5 = video block, kept internal)
  * 2x AS6C4008-55 SRAM (512Kx8 each) -> 640 KB + UMB
  * single-oscillator clock tree: 14.318 osc -> /2 (74HC74) and /3 (74HC4017),
    74HC157 select (SPEED_SEL), 74HC04 5 V buffer -> V20 CLK
...
def build(sch, lib):
    ...
    Ugate = sch.place("mini-xt:74HC32", "U10", at=(165.1, 220.98))  # OR gates
    ...
    Ugate2 = sch.place("mini-xt:74HC08", "U11", at=(205.74, 220.98))  # AND gates
    ...
    U5 = sch.place("mini-xt:74HC245", "U5", at=(165.1, 226.06))
    ...
    U7 = sch.place("mini-xt:74HC00", "U7", at=(248.92, 109.22))
    ...
    ff = sch.place("mini-xt:74HC74", "U8", at=(76.2, 60.96))   # /2
    L(ff, "VCC", "+5V", dx=0, dy=-2.54); L(ff, "GND", "GND", dx=0, dy=2.54)
    L(ff, "C", "OSC"); L(ff, "D", "CLK_QN"); L(ff, "~{Q}", "CLK_QN")
    L(ff, "Q", "CLK7"); L(ff, "~{S}", "+5V"); L(ff, "~{R}", "+5V")

    div3 = sch.place("mini-xt:74HC4017", "U9", at=(76.2, 109.22))  # /3
    L(div3, "VDD", "+5V", dx=0, dy=-2.54); L(div3, "VSS", "GND", dx=0, dy=2.54)
    L(div3, "CLK", "OSC"); L(div3, "CKEN", "GND")
    L(div3, "Q3", "DIV3_RST"); L(div3, "Reset", "DIV3_RST"); L(div3, "Q0", "CLK4")

    mux = sch.place("mini-xt:74HC157", "U12", at=(116.84, 76.2))
    ...
    buf = sch.place("mini-xt:74HC04", "U13", at=(147.32, 76.2))  # 5V buffer to CPU CLK
