...
memory). DIS_VID (GPIO42, addr_decode JP1.5 -- 2026-07-14: replaced the on-sheet
...
(CGA or MDA), letting the on-board video coexist with a second video card
...
PINS = (
    ...
    [pin("IOCHRDY", "output"),                               # card wait-states reads
     pin("DIS_VID", "input")]   # addr_decode JP1.5 level: high = card disabled (fw-read)
)
...
def build(sch, lib, expose=True):
    ...
    # DIS_VID comes from addr_decode JP1.5 (2026-07-14, was the on-sheet
    ...
    sch.text("SOFT CARD: ISA bus + power ONLY. Self-decodes 0xA0000-0xBFFFF and "
             ...
             "Disable: addr_decode JP1.5 (DIS_VID high, firmware keeps all OEs off; enabled by default). JP1 = CGA (closed) / MDA (open) window set.", (38.1, 20.32))
